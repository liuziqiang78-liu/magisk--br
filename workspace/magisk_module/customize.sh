...
ui_print "设备型号检测信息"
ui_print "=========================================="
ui_print "ro.product.vendor.name: $model"
ui_print "ro.product.model: $(getprop ro.product.model)"
ui_print "ro.build.product: $(getprop ro.build.product)"
ui_print "ro.product.device: $(getprop ro.product.device)"
ui_print "ro.product.name: $(getprop ro.product.name)"
ui_print "ro.product.board: $(getprop ro.product.board)"
ui_print "ro.build.fingerprint: $(getprop ro.build.fingerprint)"
ui_print "=========================================="
ui_print ""
...
if ! grep -q "^$model=" "$MODPATH/patch/MODULE_MAP.sh"; then
    ...
    ui_print ""
    ui_print "检测到的机型代号: $model"
    ui_print "请将此机型代号提供给开发者以添加支持"
    exit 1
else
